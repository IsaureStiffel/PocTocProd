...
class TestBaseTextCategorizationDataset(unittest.TestCase):
    ...
    def test__get_num_train_batches(self):
        ...
        # we assert that _get_num_train_samples will return 80 / batche_size = 4
    ...
    def test__get_num_test_batches(self):
        ...
        # we instantiate a BaseTextCategorizationDataset object with batch_size = 20 and train_ratio = 0.8
        base = utils.BaseTextCategorizationDataset(20, 0.8)
        # we mock _get_num_train_samples to return the value 20
        base._get_num_test_samples = MagicMock(return_value=20)
        # we assert that _get_num_train_samples will return 20 / batche_size = 1
    ...
    def test_get_index_to_label_map(self):
        ...
        # we instantiate a BaseTextCategorizationDataset object with batch_size = 20 and train_ratio = 0.8
        base = utils.BaseTextCategorizationDataset(20, 0.8)
        # we mock our list to be : ['label0', 'label1']
        base._get_label_list = MagicMock(return_value=['label0', 'label1'])
        # we assert that get_index_to_label_map() will return {0 : 'label0', 1 : 'label1'}
    ...
    def test_index_to_label_and_label_to_index_are_identity(self):
        ...
        base = utils.BaseTextCategorizationDataset(20, 0.8)
        # we mock our list to be : ['label0', 'label1']
        dict_label = ['label0', 'label1']
        base._get_label_list = MagicMock(return_value= dict_label)

        index_to_label =  base.get_index_to_label_map()
        label_to_index = base.get_label_to_index_map()

        for label in dict_label :
            predict_index = label_to_index[label]
            predict_label = index_to_label[predict_index]
            self.assertEqual(label, predict_label)

    def test_to_indexes(self):
        # TODO: CODE HERE
        # we instantiate a BaseTextCategorizationDataset object with batch_size = 20 and train_ratio = 0.8
        base = utils.BaseTextCategorizationDataset(20, 0.8)
        # we mock our list to be : {'label0' : 0, 'label1' : 1}
        dict_label = {'label0': 0, 'label1': 1}
        base._get_label_list = MagicMock(return_value=dict_label)
        labels = ['label1', 'label0', 'label1']
        self.assertEqual(base.to_indexes(labels), [1,0,1])
