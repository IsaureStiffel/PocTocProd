...
class BaseTextCategorizationDataset:
    ...
    def to_indexes(self, labels):
        ...
        # Using a list comprehension to map each label to its corresponding index
...
class LocalTextCategorizationDataset(BaseTextCategorizationDataset):
    ...
    def load_dataset(filename, min_samples_per_label):
        ...
        assert all(col in data.columns for col in expected_columns), "Columns do not match expected columns."
        ...
        def filter_tags_with_less_than_x_samples(x):
            def filter_function(df):
                ...
                frequent_tags = tag_counts[tag_counts >= x].index.tolist()
                return df[df['tag_id'].isin(frequent_tags)]
        ...
        filtered_df = filter_tag_position(0)(data)
        filtered_df = filter_tags_with_less_than_x_samples(min_samples_per_label)(filtered_df)
